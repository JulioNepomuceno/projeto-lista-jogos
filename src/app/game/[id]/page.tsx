...
export async function generateMetadata({params}: PropsParams):Promise<Metadata> {
    ...
    try {
        const response: GameProps = await fetch(`${process.env.NEXT_API_URL}/next-api/?api=game&id=${params.id}`, {cache:"no-cache"})
        .then((res)=> res.json())
        .catch(()=>{
            return{
                title:'Projeto Game'
...
async function getData(id: string) {
    ...
    try {
        const res = await fetch(`${process.env.NEXT_API_URL}/next-api/?api=game&id=${id}`, {cache:"no-cache"})
        return res.json();
    } catch (err) {
    ...
    return (
        <main className="w-full text-black">
            ...
            <Container>
                ...
                <h2 className="font-bold text-lg mt-7 mb-2">Jogo recomendados:</h2>
